function TecplotLVMeshWriteWithCentreDatauvw(tecfid,abaqusInputData,FF_f, FF_s, FF_n,FF_c,FF_r,FF_l)
...
fprintf(tecfid, 'VARIABLES = "u","v","w","FFf","FFs","FFn","FFc","FFr","FFl" \n');
fprintf(tecfid, 'ZONE\n');
...
fprintf(tecfid, '\n');
for i = 1 : NNode
   fprintf(tecfid, '%f   ', abaqusInputData.node(i,4)); 
   if mod(i,8)==0
       fprintf(tecfid, '\n');
   end
end
...
fprintf(tecfid, '\n');
for i = 1 : NNode
   fprintf(tecfid, '%f   ', abaqusInputData.node(i,5)); 
   if mod(i,8)==0
       fprintf(tecfid, '\n');
   end
end
...
fprintf(tecfid, '\n');
for i = 1 : NNode
   fprintf(tecfid, '%f   ', abaqusInputData.node(i,6)); 
   if mod(i,8)==0
       fprintf(tecfid, '\n');
   end
end
