...
for scanIndexRef = baselineScanIndex
  ...
  %  cirDir = load('cirDir.txt');
    ...
    for scanIndex = followUpScanIndex %scanIndexRef+1 :  totalScanNo %%only calculate the first and the last
        ...
        if kk==1
        cd(projectConfig_dir);
        tecFileName = sprintf('tecplotDeform_%s_to_%s_mappedmeshstandnofibre.dat',ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName );
        tecfid = fopen(tecFileName,'w');
        
        tecFileName_strain = sprintf('tecplotDG_%s_to_%s_mappedmeshstandnofibre.dat',ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName );
        tecStrainfid = fopen(tecFileName_strain,'w');
        else
        cd(projectConfig_dir);
        tecFileName = sprintf('tecplotDG_%s_to_%s_mappedmeshdensenofibre.dat',ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName );
        tecfid = fopen(tecFileName,'w');
        
        tecFileName_strain = sprintf('tecplotSrain_%s_to_%s_mappedmeshdensenofibre.dat',ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName );
        tecStrainfid = fopen(tecFileName_strain,'w');
        end
...
%
 ...
 %
        ...
        TecplotLVMeshWriteWithCentreData(tecfid,abaqusInputData,FF_c,FF_r,FF_l,FF_c, FF_r, FF_l);
        TecplotLVMeshWriteWithCentreDataStrain(tecStrainfid,abaqusInputData,EE_c,EE_r,EE_l,EE_c, EE_r, EE_l);
 ...
 %      cd(workingDir);
       ...
       cd(workingDir);
        ...
        if kk==1
        cd(projectConfig_dir);
        tecFileName = sprintf('tecplotDG_%s_to_%s_MRICoor_mappedmeshstandnofibre.dat', ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName);
        tecfid = fopen(tecFileName,'w');
        tecStrainFileName = sprintf('tecplotStrain_%s_to_%s_MRICoor_mappedmeshstandnofibre.dat', ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName);
        tecStrainfid = fopen(tecStrainFileName,'w');
        cd(workingDir);
        TecplotLVMeshWriteWithCentreData(tecfid,abaqusInputDataInMRICoor,FF_c,FF_r,FF_l,FF_c, FF_r, FF_l);
        TecplotLVMeshWriteWithCentreData(tecStrainfid,abaqusInputDataInMRICoor,EE_c,EE_r,EE_l,EE_c, EE_r, EE_l);
        fclose(tecStrainfid);
        else
         cd(projectConfig_dir);
        tecFileName = sprintf('tecplotDeform_%s_to_%s_MRICoor_mappedmeshsdensenofibre.dat', ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName);
        tecfid = fopen(tecFileName,'w');
        tecStrainFileName = sprintf('tecplotDG_%s_to_%s_MRICoor_mappedmeshdensenofibre.dat', ResultsDirAllScans(1,scanIndex).folderName, ...
            ResultsDirAllScans(1,scanIndexRef).folderName);
        tecStrainfid = fopen(tecStrainFileName,'w');
        cd(workingDir);
        TecplotLVMeshWriteWithCentreData(tecfid,abaqusInputDataInMRICoor,FF_c,FF_r,FF_l,FF_c, FF_r, FF_l);
