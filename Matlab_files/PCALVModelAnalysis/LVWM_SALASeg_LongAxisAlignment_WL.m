...
index=patientConfigs.timeInstanceSelected;
for i=1:totalScanNo
    if index==ResultsDirAllScans(i).timeInstanceSelected
    basalMovingDistance=ResultsDirAllScans(i).moveTowardsApex
    else
    end
end
...
%
%  enforced basal moving distance
%
   MovingDistEnforced=0;
...
if  BasalMovingB == 1 && abs(basalMovingDistance)>1.0e-6
    disp('Basal plane will be moved up towards annulus ring.');
    ...
    ratio = (basalMovingDistance+MovingDistEnforced)/sliceDistanceTemp;
    
    fprintf('Enforced basal plane movement distance mm         =  %8.5f\n ',MovingDistEnforced)
    fprintf('Basal plane movement distance based on image mm   =  %8.5f\n ',basalMovingDistance)
    fprintf('Distance between two slices   mm                  =  %8.5f\n ',sliceDistanceTemp)
    
    
    fprintf('Total basal plane movement ratio                  =  %8.5f\n ',ratio)
...
SAYVecNew = cross(LAXVecNew,SAXVecNew);
SAXVecNew = cross(SAYVecNew,LAXVecNew);
...
plot3(endo_cT_first_rotate(1,:),endo_cT_first_rotate(2,:), endo_cT_first_rotate(3,:),'LineStyle', '--', 'Color', 'k', 'LineWidth',2, 'Marker', 'o');hold on;
plot3(epi_cT_first_rotate(1,:),epi_cT_first_rotate(2,:), epi_cT_first_rotate(3,:),'LineStyle', '--', 'Color', 'c', 'LineWidth',2, 'Marker', '*');hold on;
...
plot3(endo_cT_first_project(1,:),endo_cT_first_project(2,:), endo_cT_first_project(3,:),'LineStyle', '--', 'Color', 'k', 'LineWidth',2, 'Marker', 'o');hold on;
plot3(epi_cT_first_project(1,:),epi_cT_first_project(2,:), epi_cT_first_project(3,:),'LineStyle', '--', 'Color', 'c', 'LineWidth',2, 'Marker', '*');hold on;
...
fprintf('Determinat of RotationMatrix      =  %8.5f\n ',det(RotationMatrix))
fprintf('Determinat of RotationMatrixNew   =  %8.5f\n ',det(RotationMatrixNew))
