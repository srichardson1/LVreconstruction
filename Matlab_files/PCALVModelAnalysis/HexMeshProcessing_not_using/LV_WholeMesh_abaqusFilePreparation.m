function LV_WholeMesh_abaqusFilePreparation(abaqusDir_reconstruction,abaqusDir_simulation)

workingDir = pwd();
cd(abaqusDir_reconstruction);
if ~exist('abaqusInputData.mat', 'file')
    error('looking for abaqusInputData.mat, not found in abaqusDir_reconstruction');
    pause;
else
...
abaqusFiberDir = 'fibreGeneration';
cd(abaqusDir_reconstruction);
if ~exist(abaqusFiberDir, 'dir')
    mkdir(abaqusFiberDir);
end
cd(abaqusFiberDir);
abaqusFiberDir = pwd();
cd(workingDir);
...
WriteAbaqusMeshInput(abaqusInputData, abaqusInputFileName, abaqusDir_simulation,workingDir);
