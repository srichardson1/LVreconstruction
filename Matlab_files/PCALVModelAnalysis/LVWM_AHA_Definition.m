...
for positionIndex = 1 : length(slices_with_6regions)
    ...
    if MRIimageFlipB == 1
       imData = imrotate(imData,90); %%only needs to rotate the image in 90 degree counterclockwise
    end
    ...
    if MRIimageFlipB == 1
       pause;
       disp('redefine the centre point of the LV cavity by clicking a point');
       [x, y, but]=myginput(1, 'crosshair');
       shapex = x;
       shapey = y;
    end
