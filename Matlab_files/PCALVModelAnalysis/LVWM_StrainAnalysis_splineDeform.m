...
sliceSelected = 5; 
%%% the range is from 1 to usuableSXSlice, including all basal and middle 
% if reversedSequence
...
cd(resultDir);
dirName = sprintf('deformRes_slice_%d', sliceSelected);
...
%%% we need to crop the images to reduce the computation time
 if imcropBool == 1
    hcrop = imshow(imgList(1,1).imData,[]);
    [I1Crop rect] = imcrop(hcrop);
 else
    cd(ResDefDir);
...
%% updated on 28th April 2022 to use AHADefinition defined for each slice.
cd(resultDir); 
load AHADefinition; % BaseMidConfig and ApexConfig
cd(workingDir);
MidConfig = BaseMidConfig(sliceSelected);
% 
