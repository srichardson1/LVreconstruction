function LVWM_SALASeg_LongAxis_ori(projectConfig_dir,projectConfig_name)
% no slicing the basal slice
close all;clc;
workingDir = pwd();
...
%% till here the rotation is done
...
for i = 1 : size(Apex_endo,2)
    ...
    endo_cT(3,:) = endo_c(3,:)-LVUpperCenter(3);
    endo_cOri = RotationMatrix*endo_cT;
    apexInnerFinal_img(:,i) = endo_cOri;
    ...
    apexOutterFinal_img(:,i) = epi_cOri;
end
% 
apexInner_point_img = mean(apexInnerFinal_img');
apexOutter_point_img = mean(apexOutterFinal_img');
...
% plot the image defined apex 
plot3(apexInner_point_img(1), apexInner_point_img(2), apexInner_point_img(3), 'Marker', '*', 'MarkerFaceColor', 'b');
plot3(apexOutter_point_img(1), apexOutter_point_img(2), apexOutter_point_img(3), 'Marker', '*', 'MarkerFaceColor', 'r');
axis equal;
...
cd(resultDir);
print(h, 'basal_rotation.png', '-dpng');
cd(workingDir);

%% till here, all data has been centred with the original basal boundaries,
%% no re-orientating and slicing the first basal slice 

DataSegSA_AlignWithLongAxis = DataSegSA; %% this suggest no changes at all to the segmented boundaries
cd(resultDir);
...
% re-orientat the first basal slice
